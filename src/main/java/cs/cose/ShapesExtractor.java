...
public class ShapesExtractor {
    ...
    private Map<Tuple3<Integer, Integer, Integer>, SupportConfidence> shapeTripletSupport;
    private Map<Integer, Integer> classInstanceCount;
    private Map<Integer, Set<Integer>> propWithClassesHavingMaxCountOne;
    private boolean isSamplingOn = false;
    private Map<Integer, Integer> propCount;
    private Map<Integer, Integer> sampledPropCount;
    private Map<Integer, Map<Integer, Set<Integer>>> classToPropWithObjTypes;
    private Map<Integer, List<Integer>> sampledEntitiesPerClass;
    private String typePredicate;
    private ValueFactory factory = SimpleValueFactory.getInstance();
    private String logfileAddress = Constants.EXPERIMENTS_RESULT;
    private Map<Integer, List<Double>> supportToRelativeSupport = new HashMap<>();
    ...
    public void constructDefaultShapes(Map<Integer, Map<Integer, Set<Integer>>> classToPropWithObjTypes) {
        ...
        Repository db = new SailRepository(new NativeStore(dbDir)); // Create a new Repository.
        ...
        try (RepositoryConnection conn = db.getConnection()) { // Open a connection to the database
            ...
            StringBuilder log = new StringBuilder("delta graph, > " + 1.0 + "%, > " + 1.0 + ",");
    ...
    public void constructShapeWithoutPruning(Map<Integer, Map<Integer, Set<Integer>>> classToPropWithObjTypes, RepositoryConnection conn) {
        ...
        try {
            if (classToPropWithObjTypes.size() > 10000) { // partition to reduce memory consumption by Model creation library
                ...
                for (List<Integer> partition : classesPartition) {
                    executor.submit(() -> {
                        ...
                        partition.forEach(encodedClassIRI -> {
                            ...
                            if (propToObjectType != null) {
                                ...
                                System.out.println("constructShapeWithoutPruning:: No properties for class: " + encoder.decode(encodedClassIRI));
                            }
                        });
                        conn.add(b.build());
                    });
    ...
    private void buildShapes(ModelBuilder b, Integer encodedClassIRI, Map<Integer, Set<Integer>> propToObjectType) {
        String classIRI = encoder.decode(encodedClassIRI);
        if (Utils.isValidIRI(classIRI)) {
            IRI subj = factory.createIRI(classIRI);
            ...
            System.out.println("constructShapeWithoutPruning::INVALID SUBJECT IRI: " + classIRI);
        }
    }
    ...
    public void constructPrunedShapes(Map<Integer, Map<Integer, Set<Integer>>> classToPropWithObjTypes, Double confidence, Integer support) {
        ...
        try (RepositoryConnection conn = db.getConnection()) { // Open a connection to the database
            ...
            HashMap<String, String> currentShapesModelStats = this.computeShapeStatistics(conn);
            StringBuilder log = new StringBuilder("delta graph , > " + confidence * 100 + "%, > " + support + ",");
            for (Map.Entry<String, String> entry : currentShapesModelStats.entrySet()) {
                log.append(entry.getValue()).append(",");
            }
            FilesUtil.writeToFileInAppendMode(log.toString(), logfileAddress);
    ...
    private void buildAndPruneShapes(Double confidence, Integer support, ModelBuilder b, Integer encodedClassIRI, Map<Integer, Set<Integer>> propToObjectType) {
        if (Utils.isValidIRI(encoder.decode(encodedClassIRI))) {
            ...
            if ((support == 1 && classInstances >= support) || (support != 1 && classInstances > support)) {
    ...
    private void prepareNodeAndPropertyShapes(Double confidence, Integer support, ModelBuilder b, Integer encodedClassIRI, Map<Integer, Set<Integer>> propToObjectType, IRI subj) {
        ...
        if (propToObjectType != null) {
            ...
            if (ConfigManager.getProperty("qse_validation_with_shNot") != null && Boolean.parseBoolean(ConfigManager.getProperty("qse_validation_with_shNot"))) {
    ...
    private Map<Integer, Set<Integer>> performPropShapePruningPositive(Integer classEncodedLabel, Map<Integer, Set<Integer>> propToObjectType, Double confidence, Integer support) {
        ...
        for (Map.Entry<Integer, Set<Integer>> entry : propToObjectType.entrySet()) {
            ...
            if (isSamplingOn) {
                ...
                supportToRelativeSupport.computeIfAbsent(support, k -> new ArrayList<>()).add(relativeSupport);
            }
        ...
        for (Integer encodedObjectType : propObjectTypes) {
            ...
            if (shapeTripletSupport.containsKey(tuple3)) {
                ...
                if ((support == 1 && sc.getConfidence() > confidence && sc.getSupport() >= support) ||
                        (isSamplingOn && support != 1 && sc.getConfidence() > confidence && sc.getSupport() > relativeSupport) ||
                        (!isSamplingOn && sc.getConfidence() > confidence && sc.getSupport() > support)) {
        ...
        propToObjectTypesLocal.forEach((prop, propObjectTypes) -> {
            ...
            if (numberOfObjectTypes == 1 && !isInstanceTypeProperty) {
                propObjectTypes.forEach(encodedObjectType -> {
                    Tuple3<Integer, Integer, Integer> tuple3 = new Tuple3<>(encoder.encode(subj.stringValue()), prop, encodedObjectType);
                    if (shapeTripletSupport.containsKey(tuple3)) {
                        if (shapeTripletSupport.get(tuple3).getSupport() == classInstanceCount.get(encoder.encode(subj.stringValue()))) {
    ...
    public void setClassToPropWithObjTypes(Map<Integer, Map<Integer, Set<Integer>>> classToPropWithObjTypes) {
